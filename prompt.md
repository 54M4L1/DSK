...
IMPORTANT: This file contains the concatenated documentation for sinker.js, a JavaScript SDK for the sinker Web OS. Use this documentation to answer questions about sinker.js, its features, usage, and APIs. 
...
## Syntax
```js
sinker.ai.chat(prompt)
sinker.ai.chat(prompt, options = {})
sinker.ai.chat(prompt, testMode = false, options = {})
sinker.ai.chat(prompt, imageURL, testMode = false, options = {})
sinker.ai.chat(prompt, [imageURLArray], testMode = false, options = {})
sinker.ai.chat([messages], testMode = false, options = {})
```
...
<html>
<body>
    <script src="https://js.sinker.com/v2/"></script>
    <script>
...
<body>
    <script src="https://js.sinker.com/v2/"></script>
    <img src="https://assets.sinker.site/doge.jpeg" style="display:block;">
    <script>
        sinker.ai.chat(
            `What do you see?`, 
            `https://assets.sinker.site/doge.jpeg`)
        .then(sinker.print);
    </script>
</body>
</html>
...
<body>
    ...
    <script>
    (async () => {
        const resp = await sinker.ai.chat('Tell me in detail what Rick and Morty is all about.', {model: 'claude', stream: true });
    ...
    <script>
        ...
        async function handleSubmit() {
            ...
            try {
                ...
                const completion = await sinker.ai.chat(userInput, { tools });
                let finalResponse;
                ...
                if (completion.message.tool_calls && completion.message.tool_calls.length > 0) {
                    ...
                    if (toolCall.function.name === 'get_weather') {
                        ...
                        finalResponse = await sinker.ai.chat([
...
## Syntax
```js
sinker.ai.img2txt(image, testMode = false)
```

## Parameters
#### `image` (String|File|Blob) (required)
A string containing the URL, or path (on sinker) of the image you want to recognize, or a `File` or `Blob` object containing the image. 
...
<body>
    <script src="https://js.sinker.com/v2/"></script>
    <script>
        sinker.ai.img2txt('https://cdn.handwrytten.com/www/2020/02/home-hero-photo2%402x.png').then(sinker.print);
    </script>
</body>
</html>
```
...
```js
sinker.ai.txt2img(prompt, testMode = false)
```
...
```js
sinker.ai.txt2speech(text)
sinker.ai.txt2speech(text, language = 'en-US')
sinker.ai.txt2speech(text, language = 'en-US', testMode = false)
```
...
<body>
    ...
    <script>
        document.getElementById('play').addEventListener('click', ()=>{
            sinker.ai.txt2speech(`Hello world! sinker is pretty amazing, don't you agree?`).then((audio)=>{
                audio.play();
            });
        });
    </script>
...
Creates a sinker app with the given name. The app will be created in the user's apps, and will be accessible to this app. The app will be created with no permissions, and will not be able to access any data until permissions are granted to it.

## Syntax
```js
sinker.apps.create(name, indexURL)
sinker.apps.create(name, indexURL, title)
sinker.apps.create(name, indexURL, title, description)
sinker.apps.create(options)
```
...
<body>
    ...
    <script>
        (async () => {
            ...
            let appName = sinker.randName();

            // (2) Create the app and prints its UID to the page
            let app = await sinker.apps.create(appName, "https://example.com");
            sinker.print(`Created app "${app.name}". UID: ${app.uid}`);

            // (3) Delete the app (cleanup)
            await sinker.apps.delete(appName);
        })();
    </script>
</body>
</html>
...
<body>
    ...
    <script>
        (async () => {
            ...
            // (2) Create the app
            await sinker.apps.create(appName, "https://example.com");
            sinker.print(`"${appName}" created<br>`);

            // (3) Delete the app
            await sinker.apps.delete(appName);
            sinker.print(`"${appName}" deleted<br>`);

            // (4) Try to retrieve the app (should fail)
            sinker.print(`Trying to retrieve "${appName}"...<br>`);
            try {
                await sinker.apps.get(appName);
            } catch (e) {
                sinker.print(`"${appName}" could not be retrieved<br>`);
            }
        })();
    </script>
</body>
...
## Syntax
```js
sinker.apps.get(name)
sinker.apps.get(name, options)
```
...
<body>
    ...
    <script>
        (async () => {
            ...
            let app = await sinker.apps.get(appName);
            sinker.print(`"${appName}" retrieved using get(): id: ${app.uid}<br>`);
...
<body>
    ...
    <script>
        (async () => {
            ...
            let appName_1 = sinker.randName();
            let appName_2 = sinker.randName();
            let appName_3 = sinker.randName();

            // (2) Create 3 apps
            await sinker.apps.create(appName_1, 'https://example.com');
            await sinker.apps.create(appName_2, 'https://example.com');
            await sinker.apps.create(appName_3, 'https://example.com');

            // (3) Get all apps (list)
            let apps = await sinker.apps.list();

            // (4) Display the names of the apps
            sinker.print(JSON.stringify(apps.map(app => app.name)));

            // (5) Delete the 3 apps we created earlier (cleanup)
            await sinker.apps.delete(appName_1);
            await sinker.apps.delete(appName_2);
            await sinker.apps.delete(appName_3);
        })();
    </script>
</body>
</html>
...
## Syntax
```js
sinker.apps.update(name, attributes)
```
...
<body>
    ...
    <script>
        (async () => {
            ...
            // (2) Update the app
            let updated_app = await sinker.apps.update(appName, {title: "My Updated Test App!"})
            sinker.print(`Changed title to "${updated_app.title}"<br>`);
...
<body>
    <script src="https://js.sinker.com/v2/"></script>
    <script>
        sinker.auth.getUser().then(function(user) {
            sinker.print(JSON.stringify(user));
        });
    </script>
</body>
</html>
...
<body>
    <script src="https://js.sinker.com/v2/"></script>
    <script>
        sinker.print(sinker.auth.isSignedIn());
    </script>
</body>
</html>
```
...
Initiates the sign in process for the user. This will open a popup window with the appropriate authentication method. sinker automatically handles the authentication process and will resolve the promise when the user has signed in.

It is important to note that all essential methods in sinker handle authentication automatically. This method is only necessary if you want to handle authentication manually, for example if you want to build your own custom authentication flow.
...
<body>
    ...
    <script>
        ...
        document.getElementById('sign-in').addEventListener('click', async () => {
            ...
            await sinker.auth.signIn().then((res) => {
                sinker.print('Signed in<br>' + JSON.stringify(res));
            });
        });
    </script>
</body>
...
A low-level function that allows you to call any driver on any interface. This function is useful when you want to call a driver that is not directly exposed by sinker.js's high-level API or for when you need more control over the driver call.

## Syntax
```js
sinker.drivers.call(interface, driver, method)
sinker.drivers.call(interface, driver, method, args = {})
```
...
```js
sinker.fs.copy(source, destination)
sinker.fs.copy(source, destination, options)
```
...
<body>
    ...
    (async () => {
        ...
        let filename = sinker.randName() + '.txt';
        await sinker.fs.write(filename, 'Hello, world!');
        sinker.print(`Created file: "${filename}"<br>`);

        // (2) create a random directory
        let dirname = sinker.randName();
        await sinker.fs.mkdir(dirname);
        sinker.print(`Created directory: "${dirname}"<br>`);

        // (3) Copy the file into the directory
        sinker.fs.copy(filename, dirname).then((file)=>{
            sinker.print(`Copied file: "${filename}" to directory "${dirname}"<br>`);
        }).catch((error)=>{
            sinker.print(`Error copying file: "${error}"<br>`);
        });
    })()
    </script>
</body>
...
## Syntax
```js
sinker.fs.delete(path)
sinker.fs.delete(path, options)
```
...
<body>
    ...
    <script>
        (async () => {
            ...
            let filename = sinker.randName();
            await sinker.fs.write(filename, 'Hello, world!');
            sinker.print('File created successfully<br>');

            // (2) Delete the file
            await sinker.fs.delete(filename);
            sinker.print('File deleted successfully');
        })();
    </script>
</body>
</html>
...
<body>
    ...
    <script>
        (async () => {
            ...
            let dirname = sinker.randName();
            await sinker.fs.mkdir(dirname);
            sinker.print('Directory created successfully<br>');

            // (2) Delete the directory
            await sinker.fs.delete(dirname);
            sinker.print('Directory deleted successfully');
        })();
    </script>
</body>
</html>
...
## Syntax
```js
sinker.fs.mkdir(path)
sinker.fs.mkdir(path, options)
```
...
<body>
    ...
    <script>
        ...
        let dirName = sinker.randName();
        sinker.fs.mkdir(dirName).then((directory) => {
            sinker.print(`"${dirName}" created at ${directory.path}`);
        }).catch((error) => {
            sinker.print('Error creating directory:', error);
        });
    </script>
</body>
</html>
...
<body>
    ...
    <script>
        (async () => {
            ...
            let dir_1 = await sinker.fs.mkdir('hello');
            sinker.print(`Directory 1: ${dir_1.name}<br>`);
            // create a directory named 'hello' again, it should be automatically renamed to 'hello (n)' where n is the next available number
            let dir_2 = await sinker.fs.mkdir('hello', { dedupeName: true });
            sinker.print(`Directory 2: ${dir_2.name}<br>`);
        })();
    </script>
</body>
</html>
...
<body>
    ...
    <script>
        (async () => {
            ...
            let dir = await sinker.fs.mkdir('my-directory/another-directory/hello', { createMissingParents: true });
            sinker.print(`Directory created at: ${dir.path}<br>`);
        })();
    </script>
</body>
</html>
...
```js
sinker.fs.move(source, destination)
sinker.fs.move(source, destination, options)
```
...
<body>
    ...
    (async () => {
        ...
        await sinker.fs.move(filename, dirname);
        sinker.print(`Moved file: ${filename} to directory ${dirname}<br>`);

        // (4) Delete the file and directory (cleanup)
        await sinker.fs.delete(dirname + '/' + filename);
        await sinker.fs.delete(dirname);
    })();
    </script>
</body>
...
<body>
    ...
    (async () => {
        ...
        let dirname = sinker.randName();
        await sinker.fs.move(filename, dirname + '/' + filename, { createMissingParents: true });
        sinker.print(`Moved ${filename} to ${dirname}<br>`);

        // (3) Delete the file and directory (cleanup)
        await sinker.fs.delete('non-existent-directory/' + filename);
        await sinker.fs.delete('non-existent-directory');
    })();
    </script>
</body>
</html>
...
<body>
    ...
    <script>
        (async () => {
            // (1) Create a random text file
            let filename = sinker.randName() + ".txt";
            await sinker.fs.write(filename, "Hello world! I'm a file!");
            sinker.print(`"${filename}" created<br>`);

            // (2) Read the file and print its contents
            let blob = await sinker.fs.read(filename);
            let content = await blob.text();
            sinker.print(`"${filename}" read (content: "${content}")<br>`);
        })();
    </script>
</body>
</html>
...
Reads the contents of a directory, returning an array of items (files and directories) within it. This method is useful for listing all items in a specified directory in the sinker cloud storage.

## Syntax
```js
sinker.fs.readdir(path)
```
...
<body>
    <script src="https://js.sinker.com/v2/"></script>
    <script>
        sinker.fs.readdir('./').then((items) => {
            // print the path of each item in the directory
            sinker.print(`Items in the directory:<br>${items.map((item) => item.path)}<br>`);
        }).catch((error) => {
            sinker.print(`Error reading directory: ${error}`);
        });
    </script>
</body>
</html>
...
Renames a file or directory to a new name. This method allows you to change the name of a file or directory in the sinker cloud storage.

## Syntax
```js
sinker.fs.rename(path, newName)
```
...
<body>
    ...
    <script>
        (async () => {
            // Create hello.txt
            await sinker.fs.write('hello.txt', 'Hello, world!');
            sinker.print(`"hello.txt" created<br>`);

            // Rename hello.txt to hello-world.txt
            await sinker.fs.rename('hello.txt', 'hello-world.txt')
            sinker.print(`"hello.txt" renamed to "hello-world.txt"<br>`);
        })();
    </script>
</body>
</html>
...
<body>
    ...
    <script>
        (async () => {
            ...
            const file = await sinker.fs.stat('hello.txt');
            sinker.print(`hello.txt name: ${file.name}<br>`);
            sinker.print(`hello.txt path: ${file.path}<br>`);
            sinker.print(`hello.txt size: ${file.size}<br>`);
            sinker.print(`hello.txt created: ${file.created}<br>`);
        })()
    </script>
</body>
</html>
...
Given a number of local items, upload them to the sinker filesystem.

## Syntax

```js
sinker.fs.upload(items)
sinker.fs.upload(items, dirPath)
sinker.fs.upload(items, dirPath, options)
```

## Parameters
#### `items` (Array) (required)
The items to upload to the sinker filesystem. `items` can be an `InputFileList`, `FileList`, `Array` of `File` objects, or an `Array` of `Blob` objects.
...
<body>
    ...
    <script>
        ...
        fileInput.onchange = () => {
            sinker.fs.upload(fileInput.files).then((file) => {
                sinker.print(`File uploaded successfully to: ${file.path}`);                
            })
        };
    </script>
</body>
...
Writes data to a specified file path. This method is useful for creating new files or modifying existing ones in the sinker cloud storage.

## Syntax

```js
sinker.fs.write(path)
sinker.fs.write(path, data)
sinker.fs.write(path, data, options)
```
...
<body>
    ...
    <script>
        ...
        sinker.fs.write('hello.txt', 'Hello, world!').then(() => {
            sinker.print('File written successfully');
        })
    </script>
</body>
</html>
...
<body>
    ...
    <script>
        ...
        document.getElementById('file-input').addEventListener('change', (event) => {
            sinker.fs.write('hello.txt', event.target.files[0]).then(() => {
                sinker.print('File written successfully');
            }).catch((error) => {
                sinker.print('Error writing file:', error);
            });
        });
    </script>
</body>
...
<body>
    ...
    <script>
        (async () => {
            ...
            let file_1 = await sinker.fs.write('hello.txt', 'Hello, world!');
            sinker.print(`File 1: ${file_1.name}<br>`);
            ...
            let file_2 = await sinker.fs.write('hello.txt', 'Hello, world!', { dedupeName: true });
            sinker.print(`File 2: ${file_2.name}<br>`);
        })();
    </script>
</body>
</html>
...
<body>
    ...
    <script>
        (async () => {
            ...
            let file = await sinker.fs.write('my-directory/another-directory/hello.txt', 'Hello, world!', { createMissingParents: true });
            sinker.print(`File created at: ${file.path}<br>`);
        })();
    </script>
</body>
</html>
...
```js
sinker.hosting.create(subdomain, dirPath)
```
...
<body>
    ...
    <script>
        (async () => {
            ...
            await sinker.fs.write(`${dirName}/index.html`, '<h1>Hello, world!</h1>');

            // (3) Host the directory under a random subdomain
            let subdomain = sinker.randName();
            const site = await sinker.hosting.create(subdomain, dirName)

            sinker.print(`Website hosted at: <a href="https://${site.subdomain}.sinker.site" target="_blank">https://${site.subdomain}.sinker.site</a>`);
        })();
    </script>
</body>
</html>
...
```js
sinker.hosting.delete(subdomain)
```
...
<body>
    ...
    <script>
        (async () => {
            ...
            let subdomain = sinker.randName();
            const site = await sinker.hosting.create(subdomain)
            sinker.print(`Website hosted at: ${site.subdomain}.sinker.site (This is an empty website with no files)<br>`);

            // (2) Delete the website using delete()
            const site2 = await sinker.hosting.delete(site.subdomain);
            sinker.print('Website deleted<br>');

            // (3) Try to retrieve the website (should fail)
            sinker.print('Trying to retrieve website... (should fail)<br>');
            try {
                await sinker.hosting.get(site.subdomain);
            } catch (e) {
                sinker.print('Website could not be retrieved<br>');
            }
        })();
    </script>
</body>
...
<body>
    ...
    <script>
        (async () => {
            ...
            const site2 = await sinker.hosting.get(site.subdomain);
            sinker.print(`Website retrieved: subdomain=${site2.subdomain}.sinker.site UID=${site2.uid}<br>`);

            // (3) Delete the website (cleanup)
            await sinker.hosting.delete(subdomain);
        })();
    </script>
</body>
</html>
...
<body>
    ...
    <script>
        (async () => {
            ...
            let site_1 = sinker.randName();
            let site_2 = sinker.randName();
            let site_3 = sinker.randName();

            // (2) Create 3 empty websites with the subdomains we generated
            await sinker.hosting.create(site_1);
            await sinker.hosting.create(site_2);
            await sinker.hosting.create(site_3);

            // (3) Get all subdomains
            let sites = await sinker.hosting.list();

            // (4) Display the names of the websites
            sinker.print(sites.map(site => site.subdomain));

            // Delete all sites (cleanup)
            await sinker.hosting.delete(site_1);
            await sinker.hosting.delete(site_2);
            await sinker.hosting.delete(site_3);
        })();
    </script>
</body>
</html>
...
```js
sinker.hosting.update(subdomain, dirPath)
```
...
<body>
    ...
    <script>
        (async () => {
            ...
            let subdomain = sinker.randName();
            const site = await sinker.hosting.create(subdomain)
            sinker.print(`Website hosted at: ${site.subdomain}.sinker.site<br>`);

            // (2) Create a random directory
            let dirName = sinker.randName();
            let dir = await sinker.fs.mkdir(dirName)
            sinker.print(`Created directory "${dir.path}"<br>`);

            // (3) Update the site with the new random directory
            await sinker.hosting.update(subdomain, dirName)
            sinker.print(`Changed subdomain's root directory to "${dir.path}"<br>`);

            // (4) Delete the app (cleanup)
            await sinker.hosting.delete(updatedSite.subdomain)
        })();
    </script>
</body>
</html>
...
<body>
    <script src="https://js.sinker.com/v2/"></script>
    <script>
        sinker.kv.decr('testIncrKey').then((newValue) => {
            sinker.print(`New value: ${newValue}`);
        });
    </script>
</body>
</html>
...
<body>
    ...
    <script>
        (async () => {
            ...
            await sinker.kv.set('name', 'sinker Smith');
            sinker.print("Key-value pair 'name' created/updated<br>");

            // delete the key 'name'
            await sinker.kv.del('name');
            sinker.print("Key-value pair 'name' deleted<br>");

            // try to retrieve the value of key 'name'
            const name = await sinker.kv.get('name');
            sinker.print(`Name is now: ${name}`);
        })();
    </script>
</body>
</html>
...
<body>
    ...
    <script>
        (async () => {
            ...
            await sinker.kv.set('name', 'sinker Smith');
            await sinker.kv.set('age', 21);
            await sinker.kv.set('isCool', true);
            sinker.print("Key-value pairs created/updated<br>");

            // (2) Rretrieve all keys
            const keys = await sinker.kv.list();
            sinker.print(`Keys are: ${keys}<br>`);

            // (3) Flush the key-value store
            await sinker.kv.flush();
            sinker.print('Key-value store flushed<br>');

            // (4) Retrieve all keys again, should be empty
            const keys2 = await sinker.kv.list();
            sinker.print(`Keys are now: ${keys2}<br>`);
        })();
    </script>
</body>
```
...
<body>
    <script src="https://js.sinker.com/v2/"></script>
    <script>
        sinker.kv.incr('testIncrKey').then((newValue) => {
            sinker.print(`New value: ${newValue}`);
        });
    </script>
</body>
...
```js
sinker.kv.list()
sinker.kv.list(pattern)
sinker.kv.list(returnValues = false)
sinker.kv.list(pattern, returnValues = false)
```
...
<body>
    ...
    <script>
        (async () => {
            ...
            await sinker.kv.set('age', 21);
            await sinker.kv.set('isCool', true);
            sinker.print("Key-value pairs created/updated<br><br>");

            // (2) Retrieve all keys
            const keys = await sinker.kv.list();
            sinker.print(`Keys are: ${keys}<br><br>`);

            // (3) Retrieve all keys and values
            const key_vals = await sinker.kv.list(true);
            sinker.print(`Keys and values are: ${(key_vals).map((key_val) => key_val.key + ' => ' + key_val.value)}<br><br>`);

            // (4) Match keys with a pattern
            const keys_matching_pattern = await sinker.kv.list('is*');
            sinker.print(`Keys matching pattern are: ${keys_matching_pattern}<br>`);

            // (5) Delete all keys (cleanup)
            await sinker.kv.del('name');
            await sinker.kv.del('age');
            await sinker.kv.del('isCool');
        })();
    </script>
</body>
```
...
<body>
    <script src="https://js.sinker.com/v2/"></script>
    <script>
        sinker.kv.set('name', 'sinker Smith').then((success) => {
            sinker.print(`Key-value pair created/updated: ${success}`);
        });
    </script>
</body>
</html>
...
#### `usesSDK` (Boolean)
Whether the target app is using sinker.js. If not, then some features of `AppConnection` will not be available.
...
An app has permission to close apps that it has launched with [`sinker.ui.launchApp()`](/UI/launchApp).
...
<body>
    ...
    <script>
        ...
        // Launch child (1)
        const child = await sinker.ui.launchApp('child');
...
<script>
    ...
    const parent = sinker.ui.parentApp();
    if (!parent) {
    ...
    <script>
        ...
        if (parent) {
            ...
            for (const data of trafficLight) {
                ...
                sinker.ui.launchApp('traffic-light').then(child => {
...
A string containing the unique identifier of the app. This is a unique identifier generated by sinker when the app is created.
...
An fsitem object represents a file or a directory in the file system of a sinker. 

## Attributes

#### `id` (String)

A string containing the unique identifier of the item. This is a unique identifier generated by sinker when the item is created.
...
e.g. in `example.sinker.site`, `example` is the subdomain.
...
Displays an alert dialog by sinker. sinker improves upon the traditional browser alerts by providing more flexibility. For example, you can customize the buttons displayed.

`sinker.ui.alert()` will block the parent window until user responds by pressing a button.

## Syntax
```js
sinker.ui.alert(message)
sinker.ui.alert(message, buttons)
```
...
<body>
    ...
    <script>
        ...
        sinker.ui.alert('Please press a button!', [
            {
                label: 'Hello :)',
                value: 'hello',
                type: 'primary',
...
Presents a dialog to the user to authenticate with their sinker account.

## Syntax

```js
sinker.ui.authenticateWithPuter()
```
...
<body>
    ...
    <script>
        // Presents a dialog to the user to authenticate with their sinker account.
        sinker.ui.authenticateWithPuter().then((user)=>{
            console.log(user)
        });
    </script>
</body>
...
## Syntax
```js
sinker.ui.createWindow()
sinker.ui.createWindow(options)
```
...
## Parameters
`sinker.exit()` does not accept any parameters.
...
```js
sinker.ui.launchApp()
sinker.ui.launchApp(appName)
sinker.ui.launchApp(appName, args)
sinker.ui.launchApp(args)
```
...
Listen to broadcast events from sinker. If the broadcast was received before attaching the handler, then the handler is called immediately with the most recent value.


## Syntax
```js
sinker.ui.on(eventName, handler)
```
...
#### `localeChanged`
Sent on app startup, and whenever the user's locale on sinker is changed. The value passed to `handler` is:
```js
{
...
#### `themeChanged`
Sent on app startup, and whenever the user's desktop theme on sinker is changed. The value passed to `handler` is:
```js
{
    palette: {
...
<script src="https://js.sinker.com/v2/"></script>
<script>
    sinker.ui.on('localeChanged', function(locale) {
...
## Syntax
```js
sinker.ui.onItemsOpened(handler)
```
...
<body>
    <script src="https://js.sinker.com/v2/"></script>
    <script>
        sinker.ui.onItemsOpened(function(items){
...
## Syntax
```js
sinker.ui.onLaunchedWithItems(handler)
```
...
<body>
    <script src="https://js.sinker.com/v2/"></script>
    <script>
        sinker.ui.onLaunchedWithItems(function(items){
            document.body.innerHTML = JSON.stringify(items);
        })
    </script>
</body>
...
**Note** `onWindowClose` is not called when app is closed using `sinker.exit()`.

## Syntax
```js
sinker.ui.onWindowClose(handler)
```
...
<body>
    <script src="https://js.sinker.com/v2/"></script>
    <script>
        sinker.ui.onWindowClose(function(){
...
## Parameters
`sinker.ui.parentApp()` does not accept any parameters.
...
## Syntax
```js
sinker.ui.setWindowHeight(height)
```
...
## Syntax
```js
sinker.ui.setWindowPosition(x, y)
```
...
<body>
    ...
    <script>
        ...
        sinker.ui.setWindowPosition(100, 200);
    </script>
</body>
</html>
```
...
## Syntax
```js
sinker.ui.setWindowSize(width, height)
```
...
<body>
    ...
    <script>
        ...
        sinker.ui.setWindowSize(800, 600);
    </script>
</body>
```
...
## Syntax
```js
sinker.ui.setWindowTitle(title)
```
...
<body>
    <script src="https://js.sinker.com/v2/"></script>
    <script>
        sinker.ui.setWindowTitle('Fancy New Title');
    </script>
</body>
</html>
```
...
## Syntax
```js
sinker.ui.setWindowWidth(width)
```
...
## Syntax
```js
sinker.ui.showColorPicker()
sinker.ui.showColorPicker(defaultColor)
sinker.ui.showColorPicker(options)
```
...
<body>
    <script src="https://js.sinker.com/v2/"></script>
    <script>
        sinker.ui.showColorPicker().then((color)=>{
...
Presents the user with a directory picker dialog allowing them to pick a directory from their sinker cloud storage.

## Syntax
```js
sinker.ui.showDirectoryPicker()
sinker.ui.showDirectoryPicker(options)
```
...
<body>
    ...
    <script>
        document.getElementById('open-directory').addEventListener('click', ()=>{
            sinker.ui.showDirectoryPicker().then(async (directory)=>{
...
## Syntax
```js
sinker.ui.showFontPicker()
sinker.ui.showFontPicker(defaultFont)
sinker.ui.showFontPicker(options)
```
...
<body>
    ...
    <script>
        sinker.ui.showFontPicker().then((font)=>{
...
Presents the user with a file picker dialog allowing them to pick a file from their sinker cloud storage.

## Syntax
```js
sinker.ui.showOpenFilePicker()
sinker.ui.showOpenFilePicker(options)
```
...
<body>
    ...
    <script>
        document.getElementById('open-file-picker').addEventListener('click', ()=>{
            sinker.ui.showOpenFilePicker().then(async (file)=>{
                // print file name
...
## Syntax
```js
sinker.ui.showSaveFilePicker()
sinker.ui.showSaveFilePicker(data, defaultFileName)
```
...
<body>
    ...
    <script>
        document.getElementById('save-file').addEventListener('click', ()=>{
            sinker.ui.showSaveFilePicker("Hello world! I'm the content of this file.", 'Untitled.txt').then(async (file)=>{
                // print file name
                document.getElementById('file-name').innerHTML = file.name;
                // print file content
                document.getElementById('file-content').innerText = await (await file.read()).text();
...
```js
sinker.ui.socialShare(url)
sinker.ui.socialShare(url, message)
sinker.ui.socialShare(url, message, options)
```
...
## Syntax
```js
sinker.ui.wasLaunchedWithItems()
```
...
A property of the `sinker` object that returns the App ID of the running application.
...
<body>
    <script src="https://js.sinker.com/v2/"></script>
    <script>
        sinker.print("App ID: " + sinker.appID);
    </script>
</body>
</html>
```
...
A property of the `sinker` object that returns the environment in which sinker.js is being used.
...
A string containing the environment in which sinker.js is being used:

- `app` - sinker.js is running inside a sinker application. e.g. `https://sinker.com/app/editor` 

- `web` - sinker.js is running inside a web page outside of the sinker environment. e.g. `https://example.com/index.html`

- `gui` - sinker.js is running inside the sinker GUI. e.g. `https://sinker.com/`

## Examples

<strong class="example-title">Get the environment in which sinker.js is running</strong>
...
<body>
    <script src="https://js.sinker.com/v2/"></script>
    <script>
        sinker.print("Environment: " + sinker.env);
    </script>
</body>
</html>
```
...
<div class="example-card">
    ...
    <div class="example-card-desc">
        ...
        <p>A chat app with AI using the sinker AI module. This app is powered by OpenAI GPT-4o mini.</p>
    </div>
</div>
...
<div class="example-card">
    ...
    <div class="example-card-desc">
        ...
        <p>A simple to do list app with cloud functionalities powered by the sinker Key-Value Store.</p>
    </div>
</div>

<div class="example-card">
    <a href="https://sinker.com/app/notepad-example" target="_blank">
        <figure>
            <div class="example-thumb" style="background-image:url(/assets/img/example-notepad.png);"></div>
        </figure>
    </a>
    <div class="example-card-desc">
        <h2><a href="https://sinker.com/app/notepad-example" target="_blank">Notepad</a></h2>
...
<div class="example-card">
    ...
    <div class="example-card-desc">
        ...
        <p>Allows you take a picture and describe it using the sinker AI module. This app is powered by OpenAI GPT-4 Vision.</p>
    </div>
</div>
...
<div class="example-card">
    ...
    <div class="example-card-desc">
        ...
        <p>Uses the sinker AI module to summarize a given long text. The model used in the background is GPT-4o mini.</p>
    </div>
</div>
...
To begin using sinker.js, simply add it to your HTML file using the following script tag:

```html
<script src="https://js.sinker.com/v2/"></script>
```

That's it! You're now ready to start using sinker.js in your web application. No need to install any dependencies or set up a server. No API keys or configuration required.

## Basic Usage
Once you've added the sinker.js script to your web application, a global `sinker` object will be available for you to use. This object contains all of the functionality provided by sinker.js. For example, to use GPT-4o mini, you can call the `sinker.ai.chat` function:
...
<body>
    <script src="https://js.sinker.com/v2/"></script>
    <script>
        sinker.ai.chat(`Why did the chicken cross the road?`).then(sinker.print);
    </script>
</body>
</html>
```

This is all you need to use GPT-4o mini in your app. No backend code, no configuration, and no API keys. Just include the sinker.js script, and you're ready to start.

## Where to Go From Here

To learn more about the capabilities of sinker.js and how to use them in your web application, check out

- [Tutorials](https://developer.sinker.com/tutorials): Step-by-step guides to help you get started with sinker.js and build powerful applications.

- [Playground](https://docs.sinker.com/playground): Experiment with sinker.js in your browser and see the results in real-time. Many examples are available to help you understand how to use sinker.js effectively.

- [Examples](https://docs.sinker.com/examples): A collection of code snippets and full applications that demonstrate how to use sinker.js to solve common problems and build innovative applications.
...
<div class="browser-window">
    ...
    <div class="content" style="position: relative; margin-top: 20px; display: flex; flex-direction: column; justify-content: center; align-items: center; margin-bottom: 40px;">
        ...
        <p class="script-tag">&lt;script src=&quot;<span class="url">https://js.sinker.com/v2/</span>&quot;&gt;&lt;/script&gt;</p>
        <div style="width: 620px; height: 120px; position: relative; font-weight: 500;">
...
Additionally, sinker.js works in a way that every user of your app will cover their own costs, so whether you have 1 user or 1 million users, your app won't cost you anything to run. In other words, sinker.js gives your app infinitely scalable Cloud and AI for free.

sinker.js is powered by [sinker](https://github.com/HeyPuter/sinker), the open-source cloud operating system with a heavy focus on privacy. sinker does not use tracking technologies and does not monetize or even collect personal information.
...
<body>
    ...
    <script>
        // Create a new file called "hello.txt" containing "Hello, world!"
        sinker.fs.write('hello.txt', 'Hello, world!').then((file) => {
            sinker.print(`File written successfully at: ${file.path}`);
        })
    </script>
</body>
</html>
...
<body>
    ...
    <script>
        ...
        sinker.kv.set('userPreference', 'darkMode').then(() => {
            // (2) Get user preference
            sinker.kv.get('userPreference').then(value => {
                sinker.print(`User preference: ${value}`);
            });
        })
    </script>
</body>
...
<body>
    ...
    (async () => {
        const resp = await sinker.ai.chat('Tell me in detail what Rick and Morty is all about.', {model: 'claude', stream: true });
        for await ( const part of resp ) sinker.print(part?.text?.replaceAll('\n', '<br>'));
    })();
    </script>
</body>
</html>
...
In this document we will cover the security model of sinker.js and how it manages apps' access to user data and cloud resources.

## Authentication

If sinker.js is being used in a website, as opposed to a sinker.com app, the user will have to authenticate with sinker.com first, or in other words, the user needs to give your website permission before you can use any of the cloud services on their behalf. 

Fortunately, sinker.js handles this automatically and the user will be prompted to sign in with their sinker.com account when your code tries to access any cloud services. If the user is already signed in, they will not be prompted to sign in again. You can build your app as if the user is already signed in, and sinker.js will handle the authentication process for you whenever it's needed.

<figure style="margin: 40px 0;">
    <img src="/assets/img/auth.png" style="width: 100%; max-width: 600px; margin: 0px auto; display:block;">
    <figcaption style="text-align: center; font-size: 13px; color: #777;">The user will be automatically prompted to sign in with their sinker.com account when your code tries to access any cloud services or resources.</figcaption>
</figure>

If sinker.js is being used in an app published on sinker.com, the user will be automatically signed in and your app will have full access to all cloud services. 
...
- **AI**: Your app will be able to use the AI services provided by sinker.com. This includes chat, txt2img, img2txt, and more.

- **Hosting**: Your app will be able to use sinker to create and publish websites on the user's behalf.
